...
import { SpecialtyIconCustomizer } from '@/components/ui/specialty-icon-customizer';
...
export function EditSpecialtyDialog({ 
  ...
  useEffect(() => {
    if (specialty) {
      ...
      setIconData({
        iconType: (specialty as any)?.iconType || 'icon',
        icon: specialty.icon || 'heart',
        iconColor: (specialty as any)?.iconColor || '#2563eb',
        imageUrl: (specialty as any)?.imageUrl || undefined
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // Auto-suggest icon if none selected and using icon mode
      const finalIcon = iconData.iconType === 'icon' 
      ...
      const response = await fetch('/api/specialties', {
        ...
        body: JSON.stringify({
          ...
          description,
          icon: finalIcon,
          iconColor: iconData.iconColor || null,
          iconType: iconData.iconType,
          imageUrl: iconData.imageUrl || null,
