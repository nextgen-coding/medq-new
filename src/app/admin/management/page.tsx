      ...
      <AdminRoute>
        <AdminLayout>
          <div className="space-y-4 sm:space-y-8">
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:justify-between sm:items-start">
              <div className="flex-1 min-w-0 sm:min-w-[280px] text-left">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center mb-4 sm:mb-6 shadow-lg shadow-blue-500/20">
                  <BookOpen className="w-7 h-7 sm:w-9 sm:h-9 text-white" />
                </div>
                <h1 className="text-2xl sm:text-4xl font-bold mb-2 sm:mb-3 bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">Gestion des spécialités</h1>
                <p className="text-muted-foreground text-sm sm:text-base leading-relaxed pr-4">Créez et gérez les spécialités médicales, organisez leurs cours et administrez les questions d'évaluation.</p>
              </div>
              <div className="flex flex-col xs:flex-row gap-2 xs:gap-3 items-stretch xs:items-center w-full sm:w-auto sm:ml-auto">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-blue-400" />
                  <Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher une spécialité..." className="pl-10 pr-8 w-full xs:w-64 sm:w-72 border-blue-200 focus:border-blue-400 focus:ring-blue-400/20" />
                ...
                <Button onClick={()=>setShowCreate(v=>!v)} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-600/25 px-4 xs:px-6 whitespace-nowrap"><PlusCircle className="h-4 w-4" /> <span className="hidden xs:inline">{showCreate ? 'Fermer' : 'Nouvelle spécialité'}</span><span className="xs:hidden">{showCreate ? 'Fermer' : 'Nouvelle'}</span></Button>
              </div>
            </div>
                ...
                {loading ? (
                  <div className="flex items-center justify-center py-8 sm:py-12">
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <p className="text-sm sm:text-base text-blue-600 font-medium">Chargement des spécialités...</p>
                    </div>
                  </div>
                ) : filtered.length === 0 ? (
                  <div className="text-center py-8 sm:py-12 px-4">
                    ...
                    <h3 className="font-semibold text-base sm:text-lg mb-2">Aucune spécialité trouvée</h3>
                    <p className="text-muted-foreground mb-4 text-sm sm:text-base">Commencez par créer votre première spécialité médicale.</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {filtered.map(s => {
                      ...
                      return (
                        <div key={s.id} className="group border border-blue-100 rounded-xl p-4 sm:p-6 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/10 transition-all duration-200 bg-gradient-to-br from-white to-blue-50/20 dark:from-background dark:to-blue-950/10 relative">
                          {!isEditing && (
                            <>
                              <Link href={`/admin/management/${s.id}`} className="block">
                                <div className="flex items-start gap-3 mb-3">
                                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center text-base sm:text-lg">
                                    {getSpecialtyIcon(s.icon)}
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <h3 className="font-semibold text-base sm:text-lg mb-1 truncate text-blue-900 dark:text-blue-100" title={s.name}>{s.name}</h3>
                                    {s.isFree && (
                                ...
                                <p className="text-xs sm:text-sm text-muted-foreground line-clamp-3 min-h-[48px] sm:min-h-[60px] leading-relaxed" title={s.description || undefined}>{s.description || 'Aucune description disponible'}</p>
                                <div className="mt-3 sm:mt-4 flex items-center text-xs sm:text-sm text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 sm:group-hover:opacity-100 transition-opacity font-medium">
                                  <span className="hidden sm:inline">Gérer les cours</span>
